...
namespace MOOS.Misc
{
    public unsafe class Thread
    {
        ...
        private void NewThread(delegate*<void> method, ulong stack_size)
        {
            ...
            if (Stack == null) Panic.Error("Thread stack allocation failed!");

            Stack->irs.cs = 0x08;
            Stack->irs.ss = 0x10;

            ulong stackPtr = (ulong)Allocator.Allocate(stack_size);
            if (stackPtr == 0) Panic.Error("Stack allocation failed!");

            Stack->irs.rsp = stackPtr + stack_size;
            Stack->irs.rsp -= 8;
